...
my $xpath = "/workflow/jobTemplateComponents//links/task/property[\@name=\"Commands\"]/property";
my $jtlName;
...
if ( $nodes->size() ) {
    foreach my $node ( $nodes->get_nodelist ) {
        $jtlName = $node->findvalue("ancestor::workflow/\@name"); # Yeah not super efficient

        my $startComponentName   = $node->findvalue("ancestor::jobTemplateComponents/\@name");
        ...
        print STDERR "Writing: $jtlName | $startComponentName | $prettyName\n" unless (defined $quiet);
        my $path = "./$jtlName/$startComponentName/$currentComponentName";
...
B<This program> will read the specified JTL file and extract the specified commands from the requested components. It will also create a file that will list all Signiant input variables that are in the script.
 The scripts will be placed in sub-directories beneath the current directory. The folder structure will be:

 <JTL>/<Start Component>/<Component>/<Command>.pl (code)
 <JTL>/<Start Component>/<Component>/<Command>.in (inputs - in XML)
